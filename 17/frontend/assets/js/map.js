...
function makePopup(school, distance, radius) {
  ...
  return `<div class="school-popup">
    ...
    <div class="sp-body">
      <div>
        ...
        <div class="sp-kuota-block">
          <div class="sp-kuota-numbers">
            <div>
              <div class="sp-kuota-main" id="sp-sisa-${school.id}" style="color:${kuotaColor}">${sisaKuota}</div>
              <div class="sp-kuota-sub" id="sp-sub-${school.id}">sisa kuota dari ${kuota} total</div>
            </div>
              <div class="sp-kuota-sisa" id="sp-pct-${school.id}" style="color:${kuotaColor}">${pct}% terisi</div>
            </div>
            <div class="sp-bar-wrap">
              <div class="sp-bar" id="sp-bar-${school.id}" style="width:${pct}%;background:${kuotaColor}"></div>
            </div>
        </div>
      </div>
      ${biayaRows}
      ...
      <button
        class="sp-more-btn"
        onclick="window._openSchoolDetail(${school.id})"
      >
        ℹ️ Info Lengkap
      </button>
    </div>
  ...
  schools.forEach(school => {
    ...
    marker.on('click', function () {
      if (!popupReady) {
        ...
        // Fetch real pendaftar count setelah popup terbuka
        import('http://localhost:8000/schools/' + school.id + '/pendaftar')
          .catch(() => null);
        fetch('http://localhost:8000/schools/' + school.id + '/pendaftar')
          .then(r => r.json())
          .then(data => _updatePopupKuota(school.id, school.kuota || 0, data?.length ?? 0))
          .catch(() => {});
      }
    });
...
/* ══════════════════════════════════════════════════════
   PICKER MAP — Set Lokasi Rumah (Profil)
══════════════════════════════════════════════════════ */
 
let _pickerMap    = null;
let _pickerMarker = null;
let _pickerCb     = null;  // callback(lat, lng) saat posisi berubah
 
/**
 * initPickerMap — inisialisasi mini-map untuk memilih lokasi rumah.
 *
 * @param {string}   containerId  - ID elemen HTML tempat map di-render
 * @param {number}   defaultLat   - lat awal marker (default: pusat Jawa Barat)
 * @param {number}   defaultLng   - lng awal marker
 * @param {Function} onPick       - callback(lat, lng) dipanggil setiap klik/drag
 */
export function initPickerMap(
  ...
  // Hancurkan instance lama jika ada (mencegah double-init)
  if (_pickerMap) {
    _pickerMap.remove();
    _pickerMap    = null;
    _pickerMarker = null;
  ...
  _pickerMarker = L.marker([defaultLat, defaultLng], {
    icon:      homeIcon,
    draggable: true,
  }).addTo(_pickerMap);
 
  // Drag selesai → callback
  _pickerMarker.on('dragend', () => {
    const { lat, lng } = _pickerMarker.getLatLng();
    _pickerCb?.(parseFloat(lat.toFixed(6)), parseFloat(lng.toFixed(6)));
  });
 
  // Klik peta → pindahkan marker + callback
...
/** Pindahkan marker picker ke koordinat tertentu (tanpa callback). */
export function setPickerMarker(lat, lng) {
  _pickerMarker?.setLatLng([lat, lng]);
  _pickerMap?.setView([lat, lng], 14);
}
 
/** Paksa ukuran picker map recalculate (diperlukan jika tab / panel awalnya hidden). */
export function invalidatePickerMap() {
  _pickerMap?.invalidateSize();
}
 
/** Hancurkan picker map (saat navigasi pindah halaman). */
...
function _updatePopupKuota(schoolId, kuota, pendaftar) {
  const sisa  = Math.max(0, kuota - pendaftar);
  const pct   = kuota > 0 ? Math.min(100, Math.round(pendaftar / kuota * 100)) : 0;
  const color = sisa < 10 ? '#c62828' : sisa < 50 ? '#e65100' : '#2e7d32';

  const sisaEl = document.getElementById(`sp-sisa-${schoolId}`);
  const subEl  = document.getElementById(`sp-sub-${schoolId}`);
  const pctEl  = document.getElementById(`sp-pct-${schoolId}`);
  const barEl  = document.getElementById(`sp-bar-${schoolId}`);

  if (sisaEl) { sisaEl.textContent = sisa; sisaEl.style.color = color; }
  if (subEl)  subEl.textContent  = `sisa kuota dari ${kuota} total`;
  if (pctEl)  { pctEl.textContent = pct + '% terisi'; pctEl.style.color = color; }
