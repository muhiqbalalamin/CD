...
from fastapi import APIRouter, Body, Depends, Header, HTTPException, Query, status, Form
...
async def beacon_logout(user_id: int = Form(...), db: Session = Depends(get_db)):
    """Endpoint khusus untuk sendBeacon — menerima form data."""
    logout_user(db, user_id)
    return {"ok": True}