...
/**
 * fetchSimulasi — ambil data simulasi untuk satu sekolah.
 * @param {number} sekolahId  - ID sekolah yang dituju
 * @param {number|null} userId - ID user yang sedang login (opsional)
 */
export async function fetchSimulasi(sekolahId, userId = null) {
  const params = userId != null ? `?user_id=${userId}` : '';
  const res = await fetch(`http://localhost:8000/simulasi/ppdb/${sekolahId}${params}`);
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  return res.json();
}
 
/**
 * cariSekolahSimulasi — resolve nama sekolah (string dari profil) → sekolah_id.
 * @param {string} nama - nama sekolah yang tersimpan di profil user
 * @returns {Array} - array { sekolah_id, nama_sekolah, kecamatan, jenjang }
 */
export async function cariSekolahSimulasi(nama) {
  try {
    const res = await fetch(
      `http://localhost:8000/simulasi/cari-sekolah?nama=${encodeURIComponent(nama)}`
    );
    if (!res.ok) return [];
    return res.json();
  } catch {
...
const WILAYAH_URL = 'http://localhost:8000/wilayah';
 
export async function fetchKabupaten() {
  try {
    const res = await fetch(`${WILAYAH_URL}/kabupaten`);
    if (!res.ok) return [];
...
export async function fetchKecamatan(kabupaten) {
  ...
  try {
    const res = await fetch(
      `${WILAYAH_URL}/kecamatan?kabupaten=${encodeURIComponent(kabupaten)}`
    );
    if (!res.ok) return [];
    return res.json();
  } catch { return []; }
...
export async function fetchKelurahan(kabupaten, kecamatan) {
  if (!kabupaten || !kecamatan) return [];
  try {
    const res = await fetch(
      `${WILAYAH_URL}/kelurahan?kabupaten=${encodeURIComponent(kabupaten)}&kecamatan=${encodeURIComponent(kecamatan)}`
    );
    if (!res.ok) return [];
    return res.json();
  } catch { return []; }
...
// 12-05-2026
const OP_BASE = 'http://localhost:8000/schools';
 
/* ── Biaya ──────────────────────────────────────────────────────── */
export async function fetchBiaya(sekolahId) {
  try {
    const res = await fetch(`${OP_BASE}/${sekolahId}/biaya`);
    if (!res.ok) return null;
    return res.json();
  } catch { return null; }
}
 
export async function saveBiaya(sekolahId, data) {
  const res = await fetch(`${OP_BASE}/${sekolahId}/biaya`, {
    method:  'PUT',
    headers: { 'Content-Type': 'application/json' },
    body:    JSON.stringify(data),
  ...
  const json = await res.json();
  if (!res.ok) throw new Error(json.detail || 'Gagal menyimpan biaya');
  return json;
}
 
/* ── Fasilitas ──────────────────────────────────────────────────── */
export async function fetchFasilitas(sekolahId) {
  try {
    const res = await fetch(`${OP_BASE}/${sekolahId}/fasilitas`);
    if (!res.ok) return [];
    return res.json();
  } catch { return []; }
}
 
export async function addFasilitas(sekolahId, data) {
  const res = await fetch(`${OP_BASE}/${sekolahId}/fasilitas`, {
    method:  'POST',
    headers: { 'Content-Type': 'application/json' },
    body:    JSON.stringify(data),
  ...
  const json = await res.json();
  if (!res.ok) throw new Error(json.detail || 'Gagal menambahkan fasilitas');
  return json;
}
 
export async function editFasilitas(fasilitasId, data) {
  const res = await fetch(`http://localhost:8000/fasilitas/${fasilitasId}`, {
    method:  'PUT',
    headers: { 'Content-Type': 'application/json' },
    body:    JSON.stringify(data),
  });
  const json = await res.json();
  if (!res.ok) throw new Error(json.detail || 'Gagal mengupdate fasilitas');
  return json;
}
 
export async function deleteFasilitas(fasilitasId) {
  const res = await fetch(`http://localhost:8000/fasilitas/${fasilitasId}`, {
    method: 'DELETE',
  });
  if (!res.ok) { const j = await res.json(); throw new Error(j.detail || 'Gagal menghapus fasilitas'); }
  return true;
}
 
/* ── Pendaftar ──────────────────────────────────────────────────── */
export async function fetchPendaftar(sekolahId) {
  try {
    const res = await fetch(`${OP_BASE}/${sekolahId}/pendaftar`);
    if (!res.ok) return [];
    return res.json();
  } catch { return []; }
}
