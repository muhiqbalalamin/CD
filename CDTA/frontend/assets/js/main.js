...
function syncRadius(val) {
  ...
  if (loc && _zonasiInitDone) {
    updateCircle(loc.lat, loc.lng, val);
  }
...
let _simKabupaten   = null;      // kabupaten user — untuk filter dropdown sekolah
let _currentSimStep = 1;         // track step aktif (1=penilaian, 2=ranking)
...
async function initSimulasiPage() {
  ...
  try {
    ...
    _simKabupaten  = profile.kabupaten ?? null;  // untuk filter dropdown sekolah
  ...
  wrap.querySelectorAll('.sim-filter-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      if (type === 'anak') {
        ...
        _currentSimStep = 1; // kembali ke step self-assessment
      } else {
        _simPilihanIdx = idx;
      }
      ...
      await loadSimulasiByFilter();
      // Re-init self assessment saat ganti anak agar checklist jenjang terupdate
      if (type === 'anak') initSelfAssessment();
    });
  });
}
...
function showSimulasiLoading(show) {
  ...
  document.getElementById('page-simulasi')?.classList.remove('sim-mode-ranking', 'sim-mode-assessment');
  ...
  if (show) {
    ...
    // Kembalikan filter dan step yang sebelumnya aktif
    filter?.classList.remove('hidden');
    showSimStep(_currentSimStep);
  }
}
...
function showSimulasiEmpty(msg, showProfileBtn = false) {
  ...
  // Tampilkan pesan di loading container
  if (loading) {
...
/* ── Inject style sekali ────────────────────────────────────────── */
function _injectSekolahComboStyles() {
  if (document.getElementById('_sekolah-combo-css')) return;
  const s = document.createElement('style');
  s.id = '_sekolah-combo-css';
  s.textContent = `
    .sekolah-combo { position: relative; flex: 1; min-width: 0; }
    .sekolah-combo-input { width: 100%; box-sizing: border-box; }
    .scd-wrap {
      position: absolute; top: calc(100% + 4px); left: 0; right: 0;
      background: #fff; border: 1px solid #cbd5e1; border-radius: 10px;
      box-shadow: 0 8px 24px rgba(0,0,0,.13); max-height: 220px;
      overflow-y: auto; z-index: 2000; display: none;
    }
    .scd-wrap.open { display: block; }
    .scd-item {
      display: flex; justify-content: space-between; align-items: center;
      padding: 9px 13px; cursor: pointer; gap: 8px;
      border-bottom: 1px solid #f1f5f9; transition: background .1s;
    }
    .scd-item:last-child { border-bottom: none; }
    .scd-item:hover, .scd-item.scd-hover { background: #eff6ff; }
    .scd-nama { font-size: 13px; font-weight: 600; color: #1e293b; flex: 1; }
    .scd-meta { font-size: 11px; color: #94a3b8; white-space: nowrap; }
    .scd-empty { padding: 14px 13px; font-size: 13px; color: #94a3b8; text-align: center; }
    .scd-hint  { padding: 8px 13px; font-size: 11px; color: #94a3b8;
                 border-bottom: 1px solid #f1f5f9; background: #f8fafc; }
  `;
  document.head.appendChild(s);
}

/* ── Helper: jenjang value → detectJenjang key ─────────────────── */
function _jenjangValToKey(val = '') {
  if (val.includes('SMK'))                           return 'smk';
  if (val.includes('SMA') || val.includes('MA'))     return 'sma';
  if (val.includes('SMP') || val.includes('MTs'))    return 'smp';
  if (val.includes('SD')  || val.includes('MI'))     return 'sd';
  return '';
}

/* ── Bind satu combo (input + dropdown) ────────────────────────── */
function _bindSekolahCombo(combo, anakIdx, slotNum) {
  const input    = combo.querySelector('.sekolah-combo-input');
  const dropdown = combo.querySelector('.scd-wrap');
  if (!input || !dropdown) return;

  let _hovIdx = -1;

  function getJenjangKey() {
    return _jenjangValToKey(_editAnak[anakIdx]?.jenjang || '');
  }

  function getFiltered(query) {
    const jKey = getJenjangKey();
    const q    = query.toLowerCase().trim();
    return getSchools().filter(s => {
      const okJ   = !jKey || detectJenjang(s.nama, s.jenjang || '') === jKey;
      const okQ   = !q   || s.nama.toLowerCase().includes(q);
      // Filter kabupaten: prioritaskan sekolah di kabupaten user
      // jika _simKabupaten tidak diset, tampilkan semua
      const okKab = !_simKabupaten || !s.kabupaten ||
                    s.kabupaten.toLowerCase() === _simKabupaten.toLowerCase();
      return okJ && okQ && okKab;
    }).slice(0, 20);
  }
  ...
  function renderDropdown(query) {
    ...
    const jKey  = getJenjangKey();
    const jLabel = _editAnak[anakIdx]?.jenjang || '';
    const kabLabel = _simKabupaten || '';

    let hint = '';
    if (jLabel && kabLabel) {
      hint = `<strong>${jLabel}</strong> di <strong>${kabLabel}</strong>`;
    } else if (jLabel) {
      hint = `<strong>${jLabel}</strong> — pilih jenjang agar tersaring`;
    } else {
      hint = 'Pilih jenjang dulu agar lebih cepat dicari';
    }

    if (!list.length) {
      dropdown.innerHTML = `<div class="scd-hint">${hint}</div><div class="scd-empty">Sekolah tidak ditemukan</div>`;
    } else {
      dropdown.innerHTML =
        `<div class="scd-hint">${hint}</div>` +
        list.map((s, i) => `
          <div class="scd-item" data-nama="${s.nama.replace(/"/g,'&quot;')}" data-sidx="${i}">
            <span class="scd-nama">${s.nama}</span>
            <span class="scd-meta">${[s.kecamatan, s.kabupaten].filter(Boolean).join(', ') || ''}</span>
          </div>`).join('');

      dropdown.querySelectorAll('.scd-item').forEach(item => {
        item.addEventListener('mousedown', e => {
          e.preventDefault();
          selectSekolah(item.dataset.nama);
        });
      });
    }
    dropdown.classList.add('open');
  ...
  function selectSekolah(nama) {
    input.value = nama;
    if (!Array.isArray(_editAnak[anakIdx].sekolahTujuan))
      _editAnak[anakIdx].sekolahTujuan = ['', '', ''];
    _editAnak[anakIdx].sekolahTujuan[slotNum] = nama;
    closeDropdown();
  }

  // Keyboard navigation
  input.addEventListener('keydown', e => {
    const items = [...dropdown.querySelectorAll('.scd-item')];
    if (!items.length) return;
    if (e.key === 'ArrowDown') {
      e.preventDefault();
      _hovIdx = Math.min(_hovIdx + 1, items.length - 1);
      items.forEach((it, i) => it.classList.toggle('scd-hover', i === _hovIdx));
      items[_hovIdx]?.scrollIntoView({ block: 'nearest' });
    } else if (e.key === 'ArrowUp') {
      e.preventDefault();
      _hovIdx = Math.max(_hovIdx - 1, 0);
      items.forEach((it, i) => it.classList.toggle('scd-hover', i === _hovIdx));
      items[_hovIdx]?.scrollIntoView({ block: 'nearest' });
    } else if (e.key === 'Enter' && _hovIdx >= 0) {
      e.preventDefault();
      selectSekolah(items[_hovIdx].dataset.nama);
  ...
  input.addEventListener('blur', () => {
    setTimeout(() => {
      closeDropdown();
      // Simpan nilai ketikan manual (bukan dari dropdown)
      if (!Array.isArray(_editAnak[anakIdx].sekolahTujuan))
        _editAnak[anakIdx].sekolahTujuan = ['', '', ''];
      _editAnak[anakIdx].sekolahTujuan[slotNum] = input.value.trim();
    }, 180);
  });
...
function renderAnakCards() {
  ...
  container.innerHTML = _editAnak.map((a, i) => {
    ...
    const jKey     = _jenjangValToKey(a.jenjang || '');
    const phPrefix = a.jenjang ? `Cari ${a.jenjang}` : 'Pilih jenjang lalu cari';
    ...
    <div class="anak-card" data-idx="${i}">
      ...
      <div class="anak-sekolah-section">
        <label class="anak-field-label">
          Sekolah tujuan
          <span class="anak-sekolah-hint">(pilih hingga 3 pilihan${jKey ? ` · filter: ${a.jenjang}` : ''})</span>
        </label>
        ${[0,1,2].map(n => `
          <div class="anak-sekolah-row">
            <span class="anak-sekolah-num">${n+1}</span>
            <div class="sekolah-combo" data-idx="${i}" data-num="${n}">
              <input class="mf-input sekolah-combo-input"
                autocomplete="off" spellcheck="false"
                placeholder="${n === 0 ? phPrefix + ' (utama)' : phPrefix + ' (opsional)'}"
                value="${(st[n] || '').replace(/"/g, '&quot;')}"/>
              <div class="scd-wrap"></div>
            </div>
          </div>`).join('')}
      </div>
    </div>`;
  ...
  // ── Bind: jenjang → update placeholder & clear invalid sekolah ──
  container.querySelectorAll('.anak-jenjang').forEach(el =>
    el.addEventListener('change', () => {
      const idx = +el.dataset.idx;
      _editAnak[idx].jenjang = el.value;
      // Re-render card agar placeholder dan hint langsung terupdate
      renderAnakCards();
    }));

  // ── Bind: sekolah combo dropdowns ──
  container.querySelectorAll('.sekolah-combo').forEach(combo => {
    const idx = +combo.dataset.idx;
    const num = +combo.dataset.num;
    _bindSekolahCombo(combo, idx, num);
  });
...
function initSimTabs() {
  document.querySelectorAll('.sim-tab').forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      document.getElementById(`sim-panel-${tab.dataset.tab}`)?.classList.add('active');
    });
  });
 
  document.getElementById('sa-btn-ke-ranking')?.addEventListener('click', () => {
    document.querySelector('.sim-tab[data-tab="ranking"]')?.click();
  });
}
 
/* ══════════════════════════════════════════════════════
   STEPPER — Show/Hide steps
══════════════════════════════════════════════════════ */
 
function showSimStep(step) {
  _currentSimStep = step;
  const page = document.getElementById('page-simulasi');
  document.getElementById('sim-step-1')?.classList.toggle('hidden', step !== 1);
  document.getElementById('sim-step-2')?.classList.toggle('hidden', step !== 2);

  // Toggle scroll lock berdasarkan step
  if (page) {
    page.classList.toggle('sim-mode-assessment', step === 1);
    page.classList.toggle('sim-mode-ranking',    step === 2);
  }
}

/* ══════════════════════════════════════════════════════
   SELF ASSESSMENT — Checklist by Jenjang
══════════════════════════════════════════════════════ */
 
const SA_DOKUMEN = {
  sd: [
    { label: 'Kartu Keluarga (KK)', sub: 'Asli dan fotokopi', pts: 10 },
    { label: 'Akta Kelahiran', sub: 'Asli dan fotokopi', pts: 10 },
    { label: 'Rapor TK / PAUD', sub: 'Atau surat keterangan dari TK', pts: 5 },
    { label: 'Pas foto terbaru', sub: 'Ukuran 3×4 dan 4×6', pts: 5 },
    { label: 'Surat Keterangan Domisili', sub: 'Jika alamat KK berbeda', pts: 5 },
  ],
  smp: [
    { label: 'Kartu Keluarga (KK)', sub: 'Asli dan fotokopi', pts: 10 },
    { label: 'Ijazah / SKHUN SD', sub: 'Asli dan fotokopi', pts: 10 },
    { label: 'Rapor SD Kelas 4–6', sub: 'Legalisir dari kepala sekolah', pts: 8 },
    { label: 'Akta Kelahiran', sub: 'Asli dan fotokopi', pts: 5 },
    { label: 'Pas foto terbaru', sub: 'Ukuran 3×4 dan 4×6', pts: 5 },
    { label: 'Sertifikat prestasi (jika ada)', sub: 'Untuk jalur prestasi', pts: 2 },
  ],
  sma: [
    { label: 'Kartu Keluarga (KK)', sub: 'Asli dan fotokopi', pts: 10 },
    { label: 'Ijazah / SKHUN SMP', sub: 'Asli dan fotokopi', pts: 10 },
    { label: 'Rapor SMP Kelas 7–9', sub: 'Legalisir dari kepala sekolah', pts: 8 },
    { label: 'Akta Kelahiran', sub: 'Asli dan fotokopi', pts: 5 },
    { label: 'Pas foto terbaru', sub: 'Ukuran 3×4 dan 4×6', pts: 5 },
    { label: 'Sertifikat prestasi (jika ada)', sub: 'Untuk jalur prestasi', pts: 2 },
  ],
  smk: [
    ...
    { label: 'Akta Kelahiran', sub: 'Asli dan fotokopi', pts: 5 },
    { label: 'Pas foto terbaru', sub: 'Ukuran 3×4 dan 4×6', pts: 5 },
    { label: 'Surat keterangan sehat', sub: 'Dari dokter / puskesmas', pts: 5 },
    { label: 'Sertifikat prestasi (jika ada)', sub: 'Untuk jalur prestasi', pts: 2 },
  ],
  other: [
    { label: 'Kartu Keluarga (KK)', sub: 'Asli dan fotokopi', pts: 10 },
    { label: 'Ijazah terakhir', sub: 'Asli dan fotokopi', pts: 10 },
    { label: 'Akta Kelahiran', sub: 'Asli dan fotokopi', pts: 5 },
    { label: 'Pas foto terbaru', sub: 'Ukuran 3×4 dan 4×6', pts: 5 },
  ],
};
...
function initSelfAssessment() {
  const anak    = _simChildren[_simAnakIdx] || {};
  const jenjang = _getJenjangKey(anak.jenjang || '');
 
  // Render checklist dokumen
  const dokWrap = document.getElementById('sa-checklist-dokumen');
  if (dokWrap) {
    const items = SA_DOKUMEN[jenjang] || SA_DOKUMEN.other;
    dokWrap.innerHTML = items.map(d => `
      <label class="sa-check-item">
        <input type="checkbox" class="sa-check" data-pts="${d.pts}"
          data-label="${d.label}"/>
        <div class="sa-check-info">
          <span class="sa-check-label">${d.label}</span>
          <span class="sa-check-sub">${d.sub}</span>
        </div>
      </label>`).join('');
  }
  ...
  document.querySelectorAll('.sa-check').forEach(cb => {
    cb.addEventListener('change', updateSAScore);
  });
 
  // Sub-step navigation
  const sub1    = document.getElementById('sa-sub-1');
  const sub2    = document.getElementById('sa-sub-2');
  const ind1    = document.getElementById('sa-sub-ind-1');
  const ind2    = document.getElementById('sa-sub-ind-2');
  const line    = document.getElementById('sa-sub-line');
  const btnNext = document.getElementById('sa-btn-sub-next');
  const btnBack = document.getElementById('sa-btn-sub-back');
 
  function goSubStep(n) {
    sub1?.classList.toggle('hidden', n !== 1);
    sub2?.classList.toggle('hidden', n !== 2);
 
    // Update dot aktif
    ind1?.classList.toggle('active', n === 1);
    ind2?.classList.toggle('active', n === 2);
    ind1?.querySelector('.sa-substep-dot')?.classList.toggle('active', n === 1);
    ind2?.querySelector('.sa-substep-dot')?.classList.toggle('active', n === 2);
 
    // Tandai dot 1 selesai saat di sub-step 2
    if (n === 2) {
      ind1?.querySelector('.sa-substep-dot')?.classList.add('done');
      line?.classList.add('done');
    } else {
      ind1?.querySelector('.sa-substep-dot')?.classList.remove('done');
      line?.classList.remove('done');
    }
  }
 
  if (btnNext) btnNext.onclick = () => goSubStep(2);
  if (btnBack) btnBack.onclick = () => goSubStep(1);
 
  // Tombol ke ranking
  const btnRanking = document.getElementById('sa-btn-ke-ranking');
  if (btnRanking) btnRanking.onclick = () => goToStep2();
 
  // Tombol kembali ke step 1
  const btnBackStep = document.getElementById('sa-btn-back');
  if (btnBackStep) btnBackStep.onclick = () => showSimStep(1);
...
function _getJenjangKey(jenjangStr = '') {
  const j = jenjangStr.toLowerCase();
  if (j.includes('smk'))            return 'smk';
  if (j.includes('sma') || j.includes('ma')) return 'sma';
  if (j.includes('smp') || j.includes('mts')) return 'smp';
  if (j.includes('sd')  || j.includes('mi'))  return 'sd';
  return 'other';
}
...
// Bobot auto status (masing-masing 15 poin)
let _saAutoZonaOk     = null;
let _saAutoJenjangOk  = null;
let _saAutoPeringkatOk = null;
 
function updateSAScore() {
  // Hitung poin checkbox
  let checkPts = 0, maxCheck = 0;
  document.querySelectorAll('.sa-check').forEach(cb => {
    const pts = parseInt(cb.dataset.pts) || 0;
    maxCheck += pts;
    if (cb.checked) checkPts += pts;
  });
 
  // Poin auto status
  let autoPts = 0;
  if (_saAutoZonaOk === true)       autoPts += 15;
  if (_saAutoJenjangOk === true)     autoPts += 15;
  if (_saAutoPeringkatOk === true)   autoPts += 15;
  const autoMax = 45;
 
  const total    = checkPts + autoPts;
  const maxTotal = maxCheck + autoMax;
  const pct      = maxTotal > 0 ? Math.round((total / maxTotal) * 100) : 0;
 
  // Update ring SVG — sekarang circumference = 314.2 (r=50)
  const circumference = 314.2;
  const offset = circumference - (pct / 100) * circumference;
  const ringColor = pct >= 80 ? '#2e7d32' : pct >= 50 ? '#e65100' : '#c62828';
 
  const fill = document.getElementById('sa-ring-fill');
  if (fill) {
  ...
  const valEl = document.getElementById('sa-score-val');
  if (valEl) valEl.textContent = pct + '%';
 
  const titleEl = document.getElementById('sa-score-title');
  const subEl   = document.getElementById('sa-score-sub');
  if (titleEl && subEl) {
    if (pct >= 80) {
      titleEl.textContent = '✅ Siap Mendaftar';
      titleEl.style.color = '#2e7d32';
      subEl.textContent   = 'Persiapan Anda sudah sangat baik';
    } else if (pct >= 50) {
      titleEl.textContent = '⚠️ Hampir Siap';
      titleEl.style.color = '#e65100';
      subEl.textContent   = 'Masih ada beberapa hal yang perlu dilengkapi';
    } else {
      titleEl.textContent = '❌ Perlu Persiapan';
      titleEl.style.color = '#c62828';
      subEl.textContent   = 'Lengkapi dokumen dan persiapan terlebih dahulu';
    }
  }
 
  // Update pins (item yang belum dicentang)
  updateSAPins();
}
 
 
/**
 * Update daftar pin "Yang perlu dilengkapi" di kolom kiri
 */
function updateSAPins() {
  const pinsEl = document.getElementById('sa-pins-list');
  if (!pinsEl) return;
 
  const pins = [];
 
  // Dari auto status
  if (_saAutoJenjangOk === false)
    pins.push({ icon: '❌', text: 'Jenjang anak tidak sesuai sekolah', type: 'error' });
  if (_saAutoZonaOk === false)
    pins.push({ icon: '⚠️', text: 'Jarak melebihi batas zona', type: 'warn' });
  if (_saAutoPeringkatOk === false)
    pins.push({ icon: '⚠️', text: 'Peringkat di luar kuota saat ini', type: 'warn' });
 
  // Dari checkbox yang belum dicentang
  document.querySelectorAll('.sa-check:not(:checked)').forEach(cb => {
    const label = cb.dataset.label ||
      cb.closest('.sa-check-item')?.querySelector('.sa-check-label')?.textContent || '';
    if (label) pins.push({ icon: '📄', text: label, type: 'doc' });
  });
 
  if (pins.length === 0) {
    pinsEl.innerHTML = '<div class="sa-pin-empty">Semua item sudah terpenuhi 🎉</div>';
    return;
  }
 
  pinsEl.innerHTML = pins.map(p => `
    <div class="sa-pin sa-pin--${p.type}">
      <span class="sa-pin-icon">${p.icon}</span>
      <span class="sa-pin-text">${p.text}</span>
    </div>`).join('');
}
 
function renderSARekomendasi(pct) {
  const el = document.getElementById('sa-rekomendasi');
  if (!el) return;
 
  const unchecked = [];
  document.querySelectorAll('.sa-check:not(:checked)').forEach(cb => {
    const label = cb.closest('.sa-check-item')?.querySelector('.sa-check-label')?.textContent;
    if (label) unchecked.push(label);
  });
 
  if (unchecked.length === 0 && _saAutoZonaOk && _saAutoJenjangOk) {
    el.classList.remove('hidden');
    el.className = 'sa-rekomendasi sa-rek-success';
    el.innerHTML = `
      <div class="sa-rek-icon">🎉</div>
      <div>
        <div class="sa-rek-title">Semua persiapan terpenuhi!</div>
        <div class="sa-rek-body">Anda siap untuk mendaftar. Lihat simulasi ranking untuk mengetahui peluang diterima.</div>
      </div>`;
    return;
  }
 
  const items = unchecked.slice(0, 3);
  if (_saAutoZonaOk === false) items.unshift('Jarak rumah melebihi zona yang disarankan');
  if (_saAutoJenjangOk === false) items.unshift('Jenjang anak tidak sesuai dengan sekolah tujuan');
 
  el.classList.remove('hidden');
  el.className = 'sa-rekomendasi sa-rek-warning';
  el.innerHTML = `
    <div class="sa-rek-icon">📌</div>
    <div>
      <div class="sa-rek-title">Yang perlu dilengkapi:</div>
      <ul class="sa-rek-list">
        ${items.map(i => `<li>${i}</li>`).join('')}
        ${unchecked.length > 3 ? `<li>… dan ${unchecked.length - 3} item lainnya</li>` : ''}
      </ul>
    </div>`;
}
...
function updateSAAutoStatus(data) {
  // ── Peringkat ──────────────────────────────────────────────
  if (data.peringkat_saya != null) {
    _saAutoPeringkatOk = data.status_saya === 'Lolos';
  } else {
    _saAutoPeringkatOk = null;
  }
 
  // ── Zona ───────────────────────────────────────────────────
  const myRow = data.peserta?.find(p => p.is_me);
  if (myRow?.jarak_km != null) {
    const batasRow = data.peserta?.[( data.kuota || 0) - 1];
    const batas    = batasRow?.jarak_km ?? null;
    _saAutoZonaOk  = batas == null || myRow.jarak_km <= batas;
  } else {
    _saAutoZonaOk = null;
  }
  ...
  // ── Kesesuaian Jenjang ────────────────────────────────────
  const anak           = _simChildren[_simAnakIdx] || {};
  const jenjangAnak    = _getJenjangKey(anak.jenjang || '');
  const jenjangSekolah = _getJenjangKey(data.jenjang_sekolah || '');
  _saAutoJenjangOk = jenjangAnak === jenjangSekolah || jenjangSekolah === 'other';
 
  // Update skor + pins (otomatis update kiri)
  updateSAScore();
}
...
function goToStep2() {
  // Update badge skor di step 2
  const pct    = parseInt(document.getElementById('sa-score-val')?.textContent) || 0;
  const badge  = document.getElementById('sim-badge-pct');
  const icon   = document.getElementById('sim-badge-icon');
  if (badge) badge.textContent = pct + '%';
  if (icon)  icon.textContent  = pct >= 80 ? '✅' : pct >= 50 ? '⚠️' : '📋';
 
  // Warna badge
  const badgeWrap = document.getElementById('sim-score-badge');
  if (badgeWrap) {
    badgeWrap.className = 'sim-score-badge ' +
      (pct >= 80 ? 'badge-green' : pct >= 50 ? 'badge-amber' : 'badge-default');
  }
 
  showSimStep(2);
 
  // Invalidate map setelah visible
  setTimeout(() => _simMap?.invalidateSize(), 150);
}

function updateSimMiniCard(data) {
  const nameEl = document.getElementById('sim-mini-name');
  const subEl  = document.getElementById('sim-mini-sub');
  const akrEl  = document.getElementById('sim-mini-akr');
 
  if (nameEl) nameEl.textContent = data.nama_sekolah || '—';
  if (subEl)  subEl.textContent  =
    [data.kecamatan, data.jenjang_sekolah,
     data.status_sekolah === 'N' ? 'Negeri' : data.status_sekolah === 'S' ? 'Swasta' : '']
    .filter(Boolean).join(' · ');
  if (akrEl && data.akreditasi) {
    const c = { A:'#1565C0', B:'#2e7d32', C:'#e65100' }[data.akreditasi] || '#546e7a';
    akrEl.innerHTML =
      `<span style="color:${c};font-size:1.3rem;font-weight:800">${data.akreditasi}</span>`;
  }
}
