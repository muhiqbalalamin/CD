...
async function loadProfileFromServer(userId, role) {
  ...
  try {
    ...
    return {
      ...
      kota: data.kota, namaAnak: data.nama_anak,
      jenjangAnak: data.jenjang_anak, sekolahTujuan: data.sekolah_tujuan,
    };
  } catch { return null; }
}
  ...
  try {
    ...
    if (role === 'admin' || role === 'sekolah') {
      ...
      payload = {
        ...
        nama_anak: frontendData.namaAnak,
        jenjang_anak: frontendData.jenjangAnak,
        sekolah_tujuan: frontendData.sekolahTujuan,
      };
    }
    const res = await fetch(`${PROFILE_API}/${userId}?role=${encodeURIComponent(role)}`, {
      method: 'PUT',
...
async function renderProfile() {
  ...
  if (isStaff) {
    ...
    set('prof-nama-anak',      data.namaAnak        || '—');
    set('prof-jenjang-anak',   data.jenjangAnak     || '—');
    set('prof-sekolah-tujuan', data.sekolahTujuan   || '—');
  }
...
function bindProfileEvents() {
  ...
  document.getElementById('btn-edit-profil-pribadi')?.addEventListener('click', () => {
    ...
    document.getElementById('pe-nama-anak').value      = d.namaAnak      || '';
    document.getElementById('pe-jenjang-anak').value   = d.jenjangAnak   || '';
    document.getElementById('pe-sekolah-tujuan').value = d.sekolahTujuan || '';
  ...
  document.getElementById('btn-simpan-profil')?.addEventListener('click', async () => {
    ...
    const g = id => document.getElementById(id)?.value.trim() || '';
    const profileData = {
      nama:g('pe-nama'), telepon:g('pe-telepon'), alamat:g('pe-alamat'), kota:g('pe-kota'),
      namaAnak:g('pe-nama-anak'), jenjangAnak:g('pe-jenjang-anak'), sekolahTujuan:g('pe-sekolah-tujuan'),
    };
    lsSaveProfile(`zj_profil_${u.username||'guest'}`, profileData);
    const uid = getSessionUserId(u);
    if (uid) await saveProfileToServer(uid, u.role, profileData);
...
async function boot() {
  ...
  if (!userJson) {
    ...
    if (lastPage && lastPage !== 'login' && lastPage !== 'register') {
      ...
      if (lastPage === 'operator') await initOperatorDashboard(user);
    } else {
      if (user.role === 'admin') {
        navigateTo('admin');
        initAdminDashboard(user);
        ...
        navigateTo('home');
        // Tampilkan popup profil belum lengkap (user biasa)
        setTimeout(() => maybeShowProfilePopup(user), 800);
      }
    }

    // Sync banner & profil page untuk semua login
    syncDashboardBanner(user);
  }
...
window.addEventListener('beforeunload', () => {
  ...
  // sendBeacon untuk request yang terjamin terkirim saat tab ditutup
  const payload = JSON.stringify({ user_id: session.user_id });
